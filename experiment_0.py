...
def fit_pacmap(matrix, n_components=2, n_neighbors=10):
    ...
    pacmap = PaCMAP(n_components=n_components)
    embedding = pacmap.fit_transform(matrix)
    return embedding
    
def fit_other_embeddings(matrix,embedding_method='cne', n_components=2):
    """
    Fit another embedding method to the given frequency matrix.
...
def main(data_path, sample_size, preprocess, label_column, use_existing_embeddings=True):
    ...
    if adata is not None:
        ...
        print("infonce embedding")
        embedding_nce= fit_other_embeddings(X_sample, embedding_method='cne', n_components=2)
        plot_embedding(embedding_nce, y_sample, title="INFONCE Embedding of Human Cell Atlas Sample", filename="Human_Cell_Atlas_Sample_INFONCE.png")
    else:
...
if __name__ == "__main__":
    ...
    parse.add_argument('--preprocess', type=bool, default=True, help='Whether to preprocess the dataset before fitting PACMAP.')
